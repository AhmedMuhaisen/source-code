...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        Validator::extend('validate_time_periods', function ($attribute, $value, $parameters, $validator) {
            $day = $parameters[0];
            $fromTimes = request()->input("day[$day]", []);
            
            foreach ($fromTimes as $index => $fromTime) {
                $toTime = request()->input("day[$day][$index]['to']", null);
                ...
                if (!is_null($fromTime) && !is_null($toTime)) {
                    ...
                    if ($fromTimestamp >= $toTimestamp) {
                        return false; // Validation failed if from >= to
                    }
                }
            }
    
            return true; // All time periods are valid
        });
