        ...
        <div class="card-body">
            <div
                ...
                <div class="align-self-center">
                    <div
                        ...
                        @include('backend.partials.table.show',['path'=> 'sale/return-purchase/list' ])
                        <div class="align-self-center d-flex gap-2">
                            @if (hasPermission('sales_return_purchase_create'))
                            <div class="align-self-center">
                                <a href="#" role="button" class="btn-add" data-bs-toggle="modal"
                                    data-bs-target="#add-purchase-return" data-bs-placement="right" data-bs-title="Add">
                        ...
                        @include('backend.partials.table.search',['path'=> 'sale/return-purchase/list' ])
                    </div>
                </div>
                {{-- @include('backend.partials.table.export',['path'=> 'sale/return-purchase/export' ]) --}}
            </div>
            <div class="table-responsive">
                <table class="table table-bordered user-table">
                    ...
                    <tbody class="tbody">
                        @foreach($data['purchaseReturns'] as $key=>$return)
                        <tr data-id="{{$return->id}}">
                            <td>{{$key+1}}</td>
                            <td>{{ date("Y-m-d", strtotime(@$return->created_at->toDateString())) }}</td>
                            <td>{{ @$return->reference_no }}</td>
                            <td>{{ @$return->purchaseReference->reference_no }}</td>
                            ...
                            <td>
                                <div class="dropdown dropdown-action">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        @if (hasPermission('sales_return_purchase_edit'))
                                        <li>
                                            <a href="{{ route('purchaseReturn.edit', $return->id) }}"
                                                class="dropdown-item"><span class="icon mr-8"><i
                                                        class="fa-solid fa-pen-to-square"></i></span>
                                                <span>{{ _trans('common.Edit') }}</span></a>
                                        </li>
                                        @endif
                                        @if (hasPermission('sales_return_purchase_delete'))
                                        <li>

                                            <a class="dropdown-item" href="javascript:void(0);"
                                                onclick="delete_row('sale/return-purchase/delete', {{ $return->id }})">
            ...
            <div class="ot-pagination pagination-content d-flex justify-content-end align-content-center py-3">
                <nav aria-label="Page navigation example">
                    <ul class="pagination justify-content-between">
                        {{$data['purchaseReturns']->appends(request()->input())->links('pagination::bootstrap-4') }}
                    </ul>
                </nav>
            </div>
            <!--  pagination end -->
...
<div id="add-purchase-return" tabindex="-1" role="dialog" aria-labelledby="add-purchase-return" aria-hidden="true"
    class="modal fade text-left">
    <div role="document" class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            {!! Form::open(['route' => 'purchaseReturn.create', 'method' => 'get']) !!}
            <div class="modal-header modal-header-image mb-3">
                <h5 class="modal-title">{{_trans('file.Add Purchase Return')}}</h5>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            <label class="form-label">{{_trans('file.Purchase Reference')}} <span class="text-danger">*</span></label>
                            <input type="text" name="reference_no" placeholder="{{_trans('file.Purchase Reference')}}" class="form-control ot-form-control ot-input">
                        </div>
                    </div>
                </div>
            ...
            <div class="container mt-3 pb-2 border-bottom">
                <div class="row">
                    ...
                    <div class="col-md-12 text-center">
                        <i style="font-size: 15px;">{{_trans('file.Purchase Return Details')}}</i>
                    </div>
                </div>
            </div>
...
@include('sale::layouts.sale-return-purchase-script')
