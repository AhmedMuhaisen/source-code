        ...
        <div class="col-md-12">
            <div class="card ot-card">
                <div class="card-header d-flex align-items-center">
                    <h4>{{_trans('file.Update Sale Return')}}</h4>
                </div>
                <div class="card-body">
                    
                    {!! Form::open(['route' => ['purchaseReturn.update', $ot_crm_return_data->id], 'method' => 'post',
                    'files' => true, 'class' => 'payment-form']) !!}
                    <div class="row">
                        <div class="col-md-12">
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <p><strong>{{ $ot_crm_return_data->reference_no }}</strong></p>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="hidden" name="warehouse_id_hidden"
                                            value="{{$ot_crm_return_data->warehouse_id}}">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="hidden" name="supplier_id_hidden"
                                            value="{{$ot_crm_return_data->supplier_id}}">
                                        <select name="supplier_id" class="form-select select2-input ot-input mb-3 modal_select2"
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label class="form-label">{{_trans('file.Account')}} <span class="text-danger">*</span></label>
                                        <select required id="account_id" name="account_id"
                                            class="form-select select2-input ot-input mb-3 modal_select2" data-live-search="true"
                                            data-live-search-style="begins">
                                            <option value="" disabled  >Select one</option>
                                            @foreach($ot_crm_account_list as $account)
                                            @if($account->id == $ot_crm_return_data->account_id)
                                ...
                                <div class="col-md-12 mt-3">
                                    ...
                                    <div class="search-box input-group">
                                        ...
                                        <input type="text" name="product_code_name" id="ot_crm_productcodeSearch"
                                            placeholder="Please type product code and select..." class="form-control ot-form-control ot-input" />
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <table id="myTable" class="table table-hover order-list">
                                             ...
                                             <tbody class="tbody">
                                                <?php
                                                    ...
                                                    ?>
                                                @foreach($ot_crm_product_return_data as $product_return)
                                                <tr>
                                                    <?php
                                                        $product_data = DB::table('sale_products')->find($product_return->product_id);
                                                        if($product_return->variant_id) {
                                                            $product_variant_data = Modules\Petcare\Entities\SaleProductVariant::select('id', 'item_code')->FindExactProduct($product_return->product_id, $product_return->variant_id)->first();
                                                        ...
                                                        $tax = DB::table('sale_taxes')->where('rate',$product_return->tax_rate)->first();
                                                        $unit_name = array();
                                                        $unit_operator = array();
                                                        ...
                                                        $product_batch_data = Modules\Petcare\Entities\SaleProductBatch::select('batch_no')->find($product_return->product_batch_id);
                                                    ?>
                                                    ...
                                                    <td>
                                                        ...
                                                        <input type="text" class="form-control ot-form-control ot-input batch-no"
                                                    ...
                                                    @endif
                                                    <td><input type="number" class="form-control ot-form-control ot-input qty" name="qty[]"
                                                            value="{{$product_return->qty}}" required step="any" /></td>
                                                    ...
                                                    <td class="tax">{{ number_format((float)$product_return->tax, 2,
                                                        '.', '') }}</td>
                                                    <td class="sub-total">{{
                                                        number_format((float)$product_return->total, 2, '.', '') }}</td>
                                            ...
                                            <tfoot class="tfoot active">
                                                ...
                                                <th></th>
                                                <th id="total-qty">{{$ot_crm_return_data->total_qty}}</th>
                                                <th></th>
                                                <th id="total-discount">{{
                                                    number_format((float)$ot_crm_return_data->total_discount, 2, '.', '')
                                                    }}</th>
                                                <th id="total-tax">{{ number_format((float)$ot_crm_return_data->total_tax,
                                                    2, '.', '')}}</th>
                                                <th id="total">{{ number_format((float)$ot_crm_return_data->total_cost, 2,
                                                    '.', '') }}</th>
                                                <th><i class="dripicons-trash"></i></th>
                                                </tr>
                                            </tfoot>
                                ...
                                <div class="col-md-2">
                                    <div class="form-group">
                                        <input type="hidden" name="total_qty"
                                            value="{{$ot_crm_return_data->total_qty}}" />
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="form-group">
                                        <input type="hidden" name="total_discount"
                                            value="{{$ot_crm_return_data->total_discount}}" />
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="form-group">
                                        <input type="hidden" name="total_tax"
                                            value="{{$ot_crm_return_data->total_tax}}" />
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="form-group">
                                        <input type="hidden" name="total_cost"
                                            value="{{$ot_crm_return_data->total_cost}}" />
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="form-group">
                                        <input type="hidden" name="item" value="{{$ot_crm_return_data->item}}" />
                                        <input type="hidden" name="order_tax"
                                            value="{{$ot_crm_return_data->order_tax}}" />
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="form-group">
                                        <input type="hidden" name="grand_total"
                                            value="{{$ot_crm_return_data->grand_total}}" />
                                    </div>
                                </div>
                            </div>
                            <div class="row mt-3">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="hidden" name="order_tax_rate_hidden"
                                            value="{{$ot_crm_return_data->order_tax_rate}}">
                                        <select class="form-select select2-input ot-input mb-3 modal_select2" name="order_tax_rate">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group mb-24">
                                        <label class="form-label">{{_trans('file.Return Note')}}</label>
                                        <textarea rows="5" class="form-control ot-form-control ot-input"
                                            name="return_note">{{$ot_crm_return_data->return_note}}</textarea>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group mb-24">
                                        ...
                                        <textarea rows="5" class="form-control ot-form-control ot-input"
                                            name="staff_note">{{$ot_crm_return_data->staff_note}}</textarea>
                                    </div>
                                </div>
                            </div>
                ...
                <form>
                    <div class="row modal-element">
                        <div class="col-md-4 form-group">
                            ...
                            <input type="number" name="edit_qty" class="form-control ot-form-control ot-input" step="any">
                        </div>
                        <div class="col-md-4 form-group">
                            <label class="form-label">{{_trans('file.Unit Discount')}}</label>
                            <input type="number" name="edit_discount" class="form-control ot-form-control ot-input" step="any">
                        </div>
                        <div class="col-md-4 form-group">
                            <label class="form-label">{{_trans('file.Unit Cost')}}</label>
                            <input type="number" name="edit_unit_cost" class="form-control ot-form-control ot-input" step="any">
                        </div>
                        <?php
                            $tax_name_all[] = 'No Tax';
                            $tax_rate_all[] = 0;
                    ...
                    @if (hasPermission('sales_return_purchase_update'))
